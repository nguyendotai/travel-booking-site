...
export default function TopDestinationsInternational() {
  const [destinations, setDestinations] = useState<Destination[]>([]);

  useEffect(() => {
    const fetchDestinations = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/destinations/featured?categoryId=2");
  ...
  return (
    <section className="my-20 max-w-7xl mx-auto px-4">
      <h2 className="text-3xl font-bold mb-10 text-center">Điểm đến nổi bật quốc tế</h2>
