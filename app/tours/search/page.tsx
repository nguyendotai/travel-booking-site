...
export default function SearchToursPage() {
      ...
      try {
        ...
        if (process.env.NEXT_PUBLIC_USE_MOCK === "true") {
          const mockFiltered = toursDomesticMock.filter((tour) => {
            ...
            const matchDate = date ? tour.startDate === date : true;

            return matchDestination && matchDate;
          });

          setTours(mockFiltered);
          return;
        }
  ...
  if (!tours.length)
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <p className="text-xl text-red-600">Không tìm thấy tour phù hợp</p>
      </div>
    );

  return (
    <div className="relative min-h-screen bg-gradient-to-r from-indigo-900 via-purple-800 to-blue-900">
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          >
            {/* Filter Title */}
            <motion.h2 className="text-xl font-extrabold text-gray-800 mb-6 flex items-center gap-2" variants={cardVariants}>
              <FaFilter /> Bộ lọc
            </motion.h2>

            {/* Price Filter */}
            <motion.div className="mb-8" variants={cardVariants}>
              <label className="block text-sm font-semibold text-gray-700 mb-3">Khoảng giá (VNĐ)</label>
              <div className="space-y-3">
                {[
                  ...
                  <label key={idx} className="flex items-center gap-2 cursor-pointer text-gray-700 hover:text-indigo-600">
                    <input
                      type="checkbox"
                      checked={selectedPriceRange?.min === range.min && selectedPriceRange?.max === range.max}
                      onChange={() => setSelectedPriceRange(selectedPriceRange?.min === range.min ? null : range)}
            ...
            <motion.div className="mb-8" variants={cardVariants}>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full p-3 rounded-lg bg-white/90 border border-gray-300 focus:ring-2 focus:ring-indigo-600 text-gray-800 text-sm appearance-none"
                >
                ...
                <ArrowDown className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
              </div>
            </motion.div>
            ...
            <motion.button
              ...
              className="w-full py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-semibold"
          ...
          <main className="lg:w-3/4">
            <motion.div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6" variants={containerVariants} initial="hidden" animate="visible">
